...
The table below contains the root mean-squared-error of the one-year ahead forecasts. These show that the TVP-VAR and the weighted VAR both perform considerably better than the unweighted VAR.

[Insert table]

Another way of illustrating the comparative performance of the models is with the cumulative RMSE, as in the figure below. 

[Insert cumulative RMSE figure]
...
The model comparison above suggests that both the TVP-VAR and analogy-weighted VAR were good candidates for a forecasting model, and may therefore be useful for estimating core inflation. Yet the two models have quite different interpretations. Under the TVP-VAR model, 
...
# References
[^1]: I would like to thank my advisor, Chris Edmond, for his input. Errors and conceptual shortcomings are all my own. Also thanks to Sue Marquez, Jim Minifie, Eugene Dubossarsky, Jack Mullumby, David Stephan, David Miller and Sarah Tan for their input at various stages in the project. 
