...
This paper describes an algorithm that makes use machine learning to estimate "analogy weights". These weights can be used to help regression models learn more from similar periods of history. The weights have two important features. First, they are metric-free; a variable's importance to the weight can vary with states of the world. Second, there are automated out-of-sample robustness checks in the construction of the weights, and so weights represent only similarities on dimensions that appear to have a robust relationship to the dependent variable. When used in a Bayesian framework with informative priors on (large) residual variance parameters, this algorithm can help increase uncertainty when the model is being asked to generate projections in a world that is not similar to the world used to estimate the model. The algorithm also requires very few tuning parameters, making it a useful turn-key solution to many time-series modelling tasks. 

The paper presents two applations of the algorithm. The first is in estimating "core" inflation---the permanent component of inflation, using a Vector Autoregression (VAR) with analogy weights. The second is an application to portfolio risk management, in particular estimating Value at Risk (VaR) for a small portfolio of stocks during the Global Financial Crisis. This second application illustrates how researchers can easily incorporate analogy weights into multivatiate GARCH models.
...
The aim of the following algorithm is to generate vector corresponding to periods $t_{1},\dots,T$ that contains weights denoting the similarity between all historical periods and period $T$. Period $T$ will typically be chosen to be the most recent period, as the model builder will generally want to build a useful model of the current state of the economy. 
...
The second sourse of variation in proximity scores is the extent of each tree in the forest; that is, the number of terminal nodes relative the the number of observations. With a single terminal node in each tree, all observations will be trivially proximate, and analogy weights will be 1. This of course would defeat the purpose of using analogy weights! If trees are grown to a full extent (the minimum terminal node size (`nodesize`) of 5 be default), then a lower bound on the average weight will be the $\frac{5}{\mbox{number of obs}}$. In practice, the average node size will be considerably larger than the minimum node size. This is because candidate splits must both decrease terminal-node variance *and* result in two terminal nodes that exceed the minimum terminal node size.
...
rf1 <- randomForest(Unemp ~ Unemp.1 + Unemp.2 + CPI.1 + CPI.2 + r.1 + r.2, data = dss, proximity = T, ntree = 5000, nodesize = 40 )
prox <- rf1$proximity[nrow(rf1$proximity),]

dss %>% mutate(prox = prox/sum(prox)) %>%
...
mod.weighted <- lm(dss$Unemp ~ ., data = indvar %>% as.data.frame, weights = prox)
mod.unweighted <- lm(dss$Unemp ~ ., data = indvar %>% as.data.frame, subset = 1:nrow(dss)>(nrow(dss)-round(sum(prox))))
...
This calculation is not straightforward; using analogy weighting can both increase and decrease the precision of parameter estimates. Because analogy scores discount dissimilar observations, they necessarily compress the support on which the parameters are estimated (see figure X above). Taken alone, this should increase uncertainty in the parameter estimates. But the entire notion of proximity weighting is that irrelevant histories are just that---and co-movements in irrelevant periods simply confound our inference. If the co-movements between dependent and independent variables during analogous histories are less variable, then the precision of parameter estimates will *increase* under analogy weighting. 

To illustrate, take a simple linear model of unemployment given first and second lagged values of unemployment, short-term interest rates and annual CPI growth. 
...
## Application 1: Changing estimates of the permanent component of inflation

To illustrate, the procedure illustrated above was carried out on quarterly US data, specifically, the unemployment rate, the rate on short-term US Treasuries, and the quarter-on-quarter percentage change of the consumer price index, taking $T$ to be the first quarter of 2015. Random forests were trained on each of these variables, taking the others to be predictors, and proximity matrices were saved. The terminal node size used was 40, about a fifth of the sample.

The plot below shows the result. Along the vertical axis is the date for which we would like historical analogies. Along the horizontal axis is the period being evaluated for similarity. The shading indicates the degree of similarity. 
...
# An application to portfolio risk management during the GFC
...
# Download the real-time data from FRED; generate some visualisations
DONE

# Write a forecaster for the time varying parameter model 
not done

# Back-test the three approaches: Regular VAR, TVP VAR, and Analogy Weighted VAR. DONE TWO


# Another cool thing to do would be to look at the Volker deflation DONE

# Do everything with realtime data from ALFRED. DONE

# Relagtive likelihood of forecast error

To do if time permitting
# Impulse response functions for a monetary policy shift at various times. 

# Calculate log of the proximity scores
...
Time-varying parameter models are commonly used in applied macroeconomics as a means of capturing unmodelled structural changes in the macroeconomy. There are three commonly used methods of generating time-variation in parameter estimates. The first is what I term an "unprincpled" approach, which includes adjusting the amount of history included in the training set, either explicitly or by weighting. A second, more principled approach is Markov switching, where an economy may flip between states, each associated with a set of parameters. Another principled approach preferred by most practitioners is to model the paremeters directly as an unobserved state that is allowed to vary over time. 

This paper presents a "semi-principled approach", a weighting methodology in which parameters are informed to a greater degree by time-periods similar a period chosen by the analyst---normally the last period. In doing so, the method captures the simplicity of a weighting approach, but one in which parameters from similar states of the world have similar values. Because there are far fewer parameters to estimate, it does not suffer from the overparameterisation issues associated with proper time-varying parameter model, and so is less reliant on the selection of priors.

The method has an important biproduct: in periods without historical precedent, the method provides very low weights on historical observations. This feature, coupled with Bayesian techniques that provide fairly tight priors on large variance parameters, allows the model to express *ignorance* when it is asked to learn from histories quite dissimilar to the present. There is little value in giving undue weight to irrelevant histories. 

The method is novel in that it uses the Random Forest, a tool from machine learning, in order to generate the weighting vector. This tool is extremely popular in applied classification and prediction tasks, though has not been widely applied to inferential econometrics. This paper is an attempt to bridge the gap.


 With the term interpreted loosely, these approaches are *purer*, in that both the data and parameters come from a generative model. Yet in most applications, time-variation in the parameters is assumed to come from (possibly non-stationary) unmodelled structural change. 