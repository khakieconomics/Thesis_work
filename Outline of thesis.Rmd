...
Hamiltonian Monte Carlo (as described in @betancourt2015hamiltonian and the @stanmanual2015) provides draws from the posterior by specifying a joint probability between parameters $\theta$ and an auxiliary variable $\rho$, $p(\rho, \theta|y, X) = p(rho|theta, X, y)\times p(\theta|X, y)$. In most applications $\rho$ comes from a multivariate normal distribution with covariance matrix $\Sigma$, $\rho\sim\mathcal{MVN}(0, \Sigma)$ and so is independent of $\theta,x,y$. Borrowing from Hamiltonian Mechanics, the joint distribution defines a Hamiltonian: 
...
\[
(\mathbf{I} - \pmb{\beta}_{1}L - \pmb{\beta}_{2}L^{2})Y_{t} = \pmb{\mu} + \pmb{epsilon}_{t}
\]

Taking expectations and inverting gives

\[
E[Y] = (\mathbf{I} - \\pmb{\beta}_{1}L - \pmb{\beta}_{2}L^{2})^{-1}\pmb{\mu} =  (\mathbf{I} - \\pmb{\beta}_{1} - \pmb{\beta}_{2})^{-1}\pmb{\mu}
\]
...
While univariate GARCH models offer a simple method of modelling conditional volatility, often a risk manager may be concerned about the correlation between assets' returns. The Constant Conditional Correlation (multivariate) GARCH model of @bollerslev1990modelling provides a very simple method of modelling this correlation. A vector of stationary variables $\mathbf{y}_{t}$ with unconditional mean $\pmb{\mu}$ and covariance matrix $\mathbf{H}_{t}$ behaves according to 
...
Assessing the performance of volatility models is not straightfoward. For one, over-predicting volatility has an opportunity cost (as it implies holding more low-yielding assets), while under-predicting volatility can lead to insufficient liquidity being held. Consequently a loss function needs to be specified, as discussed in @hou2012nonparametric. Common choices include a mean-absolute-error of the volatility forecasts, mean squared error of volatility forecasts, and heteroskedasticity-adjusted mean absolute error of @bollerslev1996periodic, which is defined as $HMAE = T^{-1}\sum_{t}\left|\frac{\epsilon_{t}^{2}}{\hat{\sigma}^{2}_t}-1\right|$, where $\epsilon_{t}^{2}$ are the squared errors of returns and $\hat{\sigma}^{2}_{t}$ is the volatility forecast for $t$. This last loss-function recognises that errors will tend to be higher in high-volatility regimes. All three loss functions are used for all three models. 

[Insert table of average loss function values]

Another important consideration is how the models performed during the most accute phase of the Financial Crisis. 

[Insert table of average loss function values for the height of the crisis]

This can be seen graphically below

[Insert 95th percentile chart]
