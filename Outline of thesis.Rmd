...
Why may similar periods be more relevant to today? This is a largely theoretical question left for further research, yet the literature contains many appealing models that give some support to the notion that the behaviour of the economy may depend on the values of macroeconomic and market variables, particularly during crises. The most prominant example of this is in Kiyotaki and Moore (1997) in which exogenous technology shocks reduce the economic value of capital and so affect asset prices. Faced with binding collateral constraints, firms cease to be able to finance investment, and so reduce their asset purchases; this reduces asset prices and amplifies the cycle. Bianchi (2010) proposes a similar mechanism, in which borrowers of foreign currency face similar collateral constraints when their home currency depreciates against the creditor's, reducing the availability of foreign credit and thus making exchange rate depreciations contractionary. In Brunnermeier and Sannikov (2013), "expert" firms can absorb small shocks by adjusting payouts and re-building capital, but under large or serially correlated shocks these firms may be forced to stop investing or liquidate their capital. Consequently, the impact of a shock in their model depends on the sign and magnitude of the shock, and the economy's distance from its steady state. 

While the channels by which these models transmit shocks vary, their implications for reduced-form modelling are similar: the behaviour of economic agents probably varies across states of the world, and there are potentially many such states. Regime-switching models as in Hamilton (1988, 1989) and as reviewed in Kuan (2002) attempt to model changes in  by allowing parameters to vary across (discrete) states. These models typically have a Markov process that allows conditional switching of states. These models have been applied in a variety of settings (see Hamilton 2005), yet like most empirical techniques they struggle with "new" states. 

As the number of potential states of the economy grows, it makes more sense to use continuous-state models, as described in Koop and Korobilis (2013). Under this approach, rather than having a set of parameters for each state of the economy, the "state" of the economy *is* the set of parameters; these are allowed to vary over time. A common implementation of these models, as in Cogley and Sargent (2001, 2005) and Koop et al (2009) models parameters as changing over time in a random walk process, rather than modelling parameter values as responding directly to changes in exogenous variables. Such a modelling choice is not necessary, but has been a preferred approach since the Minnesota Prior popularised by Doan Litterman and Sims (1984). 

These existing methods offer some real benefits to a researcher modelling unobserved structural changes. Yet the models have drawbacks. Like typical reduced-form models, discrete regime-switching models are unable to generate states that have not been observed. If the true data generating process has not revealed all her states, these methods are prone to inductive errors (Popper, 1934). In contrast, continuous-state models give positive weight to the possibility of as-yet unobserved states. But they are hard to implement, and tend to be very richly parameterised. A large number of parameters relative to the number of observations requires that informative priors must be used to estimate continuous-state models. 

The algorithm described below aims to overcome the difficulties associated with both regime-switching models and continuous-state models. It does so at a cost of moving away from a generative model. The method works by building a weighting vector that relates one period in history (normally today) with all other periods in observed history. This score serves as a "distance between histories", and is defined on the characteristics of the economy or system at that point in time. 

In practice, the distance between histories according to their characteristics would normally require the specification of a distance measure---normally a fixed metric. One approach would be to normalise all characteristics and take the Euclidian distance between all pairwise observations. But this would assume that each characteristic is equally meaningful in determining the relevant analogy---a problem if the importance of economic variables differ. More complex metrics could allow characteristics to be weighted, yet for as long as the distance is based on a fixed metric, characteristic importance weights would not vary across states of the world. 
...
The model is estimated using HMC as described above, with no weights. The model is estimated 
...
Due to volitility clustering (Mandlebrot 1963), the VaR of a portfolio will change over time. More volatile periods will imply a greater chance of loss, and so VaR will increase. A good volatility model can help managers hold liquidity at times that are most likely to be most volatile. GARCH-style models (due to Bollerslev 1986) are a common choice for this task. 

The model described below is an augmentation of the CCC-GARCH model of Bollerslev (1991). It makes two improvements. First, it allows for analogy weighting. The practical impact of allowing analogy weights is to give time-variation to the correlation matrix, so that the model will have similar correlations between asset returns across similar states of history. This sits in contrast to other popular methods like VC-GARCH of Tse and Tsui (2002) and DCC-GARCH of Engle (2002), in which correlations react to recent comovements. 
...
Multivariate GARCH models take the form 

\[
r_{t} = \mu + \Sigma_{t}^{\frac{1}{2}}\eta_{t}
\]

where $r_{t}$ is a vector of securities' log-returns between period $t-1$ and $t$, $\mu$ is a vector of mean returns, $\eta_{t}$ is a vector of white-noise, and $\Sigma_{t}$ is a time-varying conditional covariance matrix. The many multivariate GARCH models provide different specifications for the time-variation in the covariance matrix. One popular method, the CCC-GARCH of Bollerslev (1990) models covariance by decomposing the covariance into a dynamic scale vector $\tau_{t}$ and static correlation matrix $\Omega$.

\[
\Sigma_{t} = \mbox{diag}(\tau_{t})\Omega\mbox{diag}(tau_{t})
\]

The scale terms are then given a GARCH specification: 

\[
\tau_{t} = c + \mbox{diag}(A)\tau_{t-1} + \mbox{diag}(B)r_{t-1}
\]

To provide Bayesian inference about the model, prior distributions are given to the parameters. 
