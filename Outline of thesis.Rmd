...
How can analogy weights help to improve the relevance of parameter estimation? This will depend on the complexity of the model.
...
For historical data points that are given little weight, this has the effect of reducing their importance in the estimation of $\theta$. If a data point is given zero weight, it has no impact on the estimation of the parameter. It is important to adjust the standard errors of parameters estimates when using weighted maximum likelihood. This will be affected by the weights in two ways: first, the variance-covariance matrix of $\theta$ will be a weighted variance covariance matrix, and second, there will be fewer effective observations. Analytically, the (weighted) Fisher Information matrix 
...
\[
\Sigma_{\beta} \approx \frac{1}{\sum_{i}\omega_{i}}I(\theta)^{-1}
\]
...
Where $T$ is the "kinetic energy" and $V$ is the "potential energy". Given an some parameter value $\theta_{t}$, the next parameter draw $\theta_{t+1}$ is generated as so. First, a value of $\rho$ is drawn. 
