...
This integration has the effect of "steering" $\theta$ towards parts of its distribution with higher density. Consequently, $\theta$ spends a proportionately large amount of time towards the mode of its distribution, and less time at the tails. At the end of $L$ steps, there will be some numerical error. This is because while the surface of the density of $\theta$ is smooth, movements along it in each step are straight (the error increases with step size). A Metropolis step is therefore used to decide whether to keep the draw, where the probability of keeping a draw that started at $(\rho_{0}, \theta_{0})$ and arrived at $(\rho_{L}, \theta_{L})$ after $L$ steps is:

\[
\min(1, \exp(H(\rho_{0}, \theta_{0}) - H(\rho_{L}, \theta_{L})))
\]

The `stan` software package uses an implementation of HMC known as the No U-Turn Sampler (NUTS) of Hoffman and Gelman (2014). This algorithm automatically chooses the three control parameters, $L$, $\epsilon$ and $\Sigma$, but otherwise performs HMC as described above. 

A vector of weights $\omega$ can be included in Hamiltonian Monte Carlo by weighting each observation's impact on the potential energy term $V_{\omega} = -\omega\log p(\theta|y, X)$. This is analogous to the case of weighting observations under maximum likelihood, although the potential energy term contains prior information also. 
